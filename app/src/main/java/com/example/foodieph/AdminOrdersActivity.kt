...
class AdminOrdersActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Fetch all orders across all users via collectionGroup (no server orderBy — avoids index requirement)
        db.collectionGroup("orders")
            ...
            .addOnSuccessListener { snapshot ->
                val orders = snapshot.documents.mapNotNull { doc ->
                    ...
                    val placedAtMs = ts?.toDate()?.time ?: 0L
                    AdminOrderSummary(
                        orderId = serial,
                ...
                }.sortedByDescending { it.placedAtMs }
                rv.adapter = AdminOrdersAdapter(orders)
            }
            .addOnFailureListener {
                rv.adapter = AdminOrdersAdapter(emptyList())
            }
    }
}
